// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.dev.implementation;

import com.azure.dev.fluent.models.PreviewRunInner;
import com.azure.dev.models.PreviewRun;

public final class PreviewRunImpl implements PreviewRun {
    private PreviewRunInner innerObject;

    private final com.azure.dev.DevManager serviceManager;

    PreviewRunImpl(PreviewRunInner innerObject, com.azure.dev.DevManager serviceManager) {
        this.innerObject = innerObject;
        this.serviceManager = serviceManager;
    }

    public String finalYaml() {
        return this.innerModel().finalYaml();
    }

    public PreviewRunInner innerModel() {
        return this.innerObject;
    }

    private com.azure.dev.DevManager manager() {
        return this.serviceManager;
    }
}
