// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
// Code generated by Microsoft (R) AutoRest Code Generator.

package com.azure.dev.implementation;

import com.azure.dev.fluent.models.AttachmentInner;
import com.azure.dev.models.Attachment;
import com.azure.dev.models.ReferenceLinks;

public final class AttachmentImpl implements Attachment {
    private AttachmentInner innerObject;

    private final com.azure.dev.DevManager serviceManager;

    AttachmentImpl(AttachmentInner innerObject, com.azure.dev.DevManager serviceManager) {
        this.innerObject = innerObject;
        this.serviceManager = serviceManager;
    }

    public ReferenceLinks links() {
        return this.innerModel().links();
    }

    public String name() {
        return this.innerModel().name();
    }

    public AttachmentInner innerModel() {
        return this.innerObject;
    }

    private com.azure.dev.DevManager manager() {
        return this.serviceManager;
    }
}
